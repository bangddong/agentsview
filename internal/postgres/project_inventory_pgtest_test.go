...
func TestPGProjectInventoryCrossArchiveIsolation(t *testing.T) {
	...
	assert.Equal(t, 1, inv.GovernedSessions,
		"only archive B's own session is governed by archive B's rule; "+
			"archive A has no rule of its own on shared-machine, so its "+
			"session must not be governed by archive B's identical-path rule")
	...
	// Directly exercise the (source_archive_id, machine) tuple filter in
	...
	assert.NotContains(t, candidateIDs, "a-session",
		"archive A has no enabled mapping of its own; the (source_archive_id, "+
			"machine) tuple filter must not admit its session just because "+
			"archive B has an enabled mapping on the same machine name")
