...
// snapshot group spans an old (2020) and a new (2025) session, and a narrow
// one-day Activity-scoped query against the SQLite side (the only backend
// with that method) sees only the new session, confirming the archive-wide
// method is not merely reproducing a wide-enough window. Since the
// differential assertion above already proves the PG and SQLite archive-wide
// results are identical, this transitively proves PG's result is
// archive-wide too.
func TestPGWorktreeCandidatesArchiveWideMatchesSQLite(t *testing.T) {
	...
	require.NoError(t, err, "ResolveQuery")
	scoped, err := localDB.ListWorktreeReclassificationCandidates(ctx,
		db.WorktreeCandidateRequest{
			Filter:            db.AnalyticsFilter{Timezone: "UTC"},
			Query:             q,
			ClickedProject:    export.SafeProjectDisplayLabel(project),
			ClickedProjectKey: projects[project].ProjectKey,
		})
	require.NoError(t, err, "local ListWorktreeReclassificationCandidates")
	require.Len(t, scoped, 5)
	assert.Equal(t, 1, scoped[0].ContributingSessions,
		"a narrow one-day window only sees the 2025 session, unlike the "+
			"archive-wide result's 2")
}
