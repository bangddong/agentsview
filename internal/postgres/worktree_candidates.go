...
// source_database_generation, source_session_id) rather than session id
// alone, so a ROW_NUMBER dedup picks one snapshot per (source_archive_id,
// session id) -- in steady state a session has rows from a single
// generation (a full push clears an archive's whole snapshot scope before
// re-inserting under the current generation), but the dedup keeps the
// query correct even if two generations briefly coexist.
