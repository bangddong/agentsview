...
func (p *hermesProvider) parseStateMember(
	...
	// unchanged member; otherwise every pass reparses every member and
	// reconciliation work scales with total member count.
...
// seedHermesMemberCores reads every state.db member once on one shared
// connection and caches each member's fingerprint core for the rest of the
// pass. The per-reconciliation once keeps the bulk read to one per state.db
// per pass even when parallel fingerprint workers miss concurrently or a
// member read error leaves individual entries unseeded.
...
func seedHermesMemberCoresLocked(ctx context.Context, stateDB string) error {
	...
	for _, id := range ids {
		if err := cacheHermesMemberCore(
			ctx, conn, stateDB, id, VirtualSourcePath(stateDB, id),
		); err != nil {
			return err
		}
	}
