...
type WorktreeCandidateRequest struct {
...
// ListWorktreeReclassificationCandidates returns the machine/path groups that
// contributed to one opaque Activity project row. The Activity filter and
// range select candidate sessions; this method does not estimate the eventual
// full-archive rewrite performed by preview and apply.
func (db *DB) ListWorktreeReclassificationCandidates(
	ctx context.Context,
	request WorktreeCandidateRequest,
) ([]WorktreeReclassificationCandidate, error) {
	if strings.TrimSpace(request.ClickedProjectKey) == "" {
		return nil, fmt.Errorf("clicked_project_key is required")
	}
	request.Filter.IncludeSubagents = true
	request.Filter.IncludeForks = true
	start, end := activityReportRangeBoundsUTC(request.Query)
	sessions, _, err := db.activityReportSessions(ctx, request.Filter, start, end)
	if err != nil {
		return nil, err
	}
	labels := activityReportProjectLabels(sessions)
	projects, err := db.BuildProjectIdentityMap(ctx, labels)
	if err != nil {
		return nil, err
	}
	...
	for _, session := range sessions {
		...
		if export.SafeProjectDisplayLabel(session.Project) != request.ClickedProject ||
			entry.ProjectKey != request.ClickedProjectKey {
			continue
		}
		selectedIDs = append(selectedIDs, session.SessionID)
...
// It mirrors ListWorktreeReclassificationCandidates's post-selection
// pipeline exactly, differing only in how candidate sessions are chosen.
...
// over an already-selected set of session IDs. Both
// ListWorktreeReclassificationCandidates and ListArchiveWorktreeCandidates
// call it after choosing their session set by different means.
