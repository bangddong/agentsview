...
// session, and a narrow one-day Activity-scoped query against the SQLite
// side (the only backend with that method) sees only the new session,
// confirming the archive-wide method is not merely reproducing a
// wide-enough window. Since the differential assertion above already proves
// the DuckDB and SQLite archive-wide results are identical, this
// transitively proves DuckDB's result is archive-wide too.
func TestDuckWorktreeCandidatesArchiveWideMatchesSQLite(t *testing.T) {
	...
	scoped, err := local.ListWorktreeReclassificationCandidates(ctx,
